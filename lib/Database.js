...
Database.prototype.makeRest = function(options) {
	var self = this, segpat = /^([^/]+)(\/|$)/;
	options = options || {};
	
	options = {
		prefix: new RegExp('^' +
			('/'+(options.prefix||'')+'/').replace(/\/+/g, '/'). 	// wrap with slash
				replace(/([.?*+^$[\]\\(){}|-])/g, "\\$1")	// escape
		)
	};
	...
	return function(req, res) {
		...
		if(!options.prefix.test(path)) return failure("Not an API call.");
		
		path = path.replace(options.prefix, "");
