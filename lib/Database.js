...
Database.prototype.createTables = function(force) {
	var i;
	for(i in this.tables) {
		...
		this.createTable(this.tables[i], force);
	}
};

Database.prototype.createTable = function(table, force) {
...
Database.prototype.toSql = function (query) {
	...
	if(query instanceof Array) {
		return "(" +
			query.map(function(val) { return self.toSql(val); }).join(",") +
		")";
	}
	...
	if(query.verb == "select") {
		...
		sql = "INSERT INTO " + nd + query.table + nd +
			" SET " + set(query.data) + " ON DUPLICATE KEY UPDATE " +
			set(query.data);
