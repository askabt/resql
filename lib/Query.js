...
Query.prototype.execute = function() {
	...
	this.database.query(
		...
		function(err, value) {
			...
			/*function assemble(objs, root, map, ctx) {
				console.log(ctx);
				var i, ref, id = objs[ctx].id;
				
				if(!root[id]) root[id] = objs[ctx];
				
				if(typeof map == "object") for(i in map) {
					if(i == "_ref" || i == "_out") continue;
					if(!root[id][i]) root[id][i] = {};
					console.log(i, map[i]);
					assemble(objs, root[id][i], map[i], map[i]._ref);
				}
			}
			
			function cleanup(root, map) {
				if(map._out) {
